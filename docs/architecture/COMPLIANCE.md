...
| CI runs the linters the repo claims | ◐ | `dotnet format --verify-no-changes` added but **advisory**. `.editorconfig` arrived after the code did, and the existing source uses aligned assignments that `dotnet format` collapses. Making it blocking before that pass lands would mean permanently red CI, which teaches people to ignore CI. One line to flip once the formatting commit exists. |
